...
abstract class Base extends TestCase {
    ...
    protected function getProtectedProperty($object_name, $name ) {
        $reflection = new \ReflectionClass($object_name);
        $property   = $reflection->getProperty($name);

        return $property->getValue($object_name);
    }

    protected function setProtectedProperty($object_name, $name, $value ): void {
        $reflection = new \ReflectionClass($object_name);
        $property   = $reflection->getProperty($name);

        $property->setValue($object_name, $value);
    }

    protected function callProtectedMethod($object_name, $name, array $args = [] ) {
        $reflection = new \ReflectionClass($object_name);
        $method     = $reflection->getMethod($name);

        return $method->invokeArgs($object_name, $args);
    }
